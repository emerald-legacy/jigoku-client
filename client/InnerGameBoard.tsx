...
import ConflictDeclarationOverlay from "./GameComponents/ConflictDeclarationOverlay";
...
type ContextMenuOption =
    | { text: string; onClick: (...args: unknown[]) => unknown }
...
export interface InnerGameBoardProps {
    ...
    user?: User;
    username?: string;
    dispatch: AppDispatch | ((action: unknown) => unknown);
...
export function InnerGameBoard(props: InnerGameBoardProps) {
    ...
    const onCardClick = (card: CardType) => {
        if(card && card.uuid) {
            ...
            const controllerName = typeof card.controller === "string" ? card.controller : card.controller.name;
            sendGameMessage("facedownCardClicked", card.location, controllerName, card.isProvince);
        }
    };
    ...
    const onCommand = (command: string, arg: unknown, uuid: string, method: string) =>
    ...
    useEffect(() => {
        ...
        if(currentGame.started) {
            if(Object.values(currentGame.players).find((p: Player) => p.name === username)) {
    ...
    return (
        <div className="game-board">
            ...
            <ConflictDeclarationOverlay conflict={ currentGame.conflict } />
